...
router.get("/:id", async (req, res) => {
  ...
  if (!photo) {
    return res
      .status(400)
      .json({ error: `Fail to find a photo with the id ${id}` });
  }
...
router.delete("/:id", async (request, response) => {
  ...
  if (!status) {
    return response
      .status(400)
      .json({ error: `The photo with the id ${id} doesn't exist` });
  }
...
router.put("/:id", async (request, response) => {
  ...
  if (!photo) {
    return response
      .status(400)
      .json({
        error: `Fail to update or nothing changed on photo with the id ${id}`,
      });
  }
