...
router.post("/", async (request, response) => {
  ...
  if (!contact) {
    return response
      .status(400)
      .json({ error: "Fail to create product category" });
  }
...
router.delete("/:id", async (request, response) => {
  ...
  if (!status) {
    return response
      .status(400)
      .json({ error: `The product category with the id ${id} doesn't exist` });
  }
...
router.put("/:id", async (request, response) => {
  ...
  if (!client) {
    return response.status(400).json({
      error: `Fail to update or nothing changed on product category with the id ${id}`,
    });
  }
